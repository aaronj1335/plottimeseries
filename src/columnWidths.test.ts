...
test('apportionColumnWidths', async (t) => {
  await t.test('never totals more than the measured width', () => {
  ...
  await t.test('gives the leftover pixels to the largest remainders', () => {
  ...
  await t.test('leaves whole numbers alone', () => {
    assert.deepStrictEqual(apportionColumnWidths([40, 60, 100]), [40, 60, 100]);
  });

  await t.test('preserves overflow that the content really needs', () => {
  ...
  await t.test('handles no columns', () => {
